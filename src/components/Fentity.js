import React, { Component } from 'react';
import { Link } from 'react-router-dom';
import '../styles/fentity.css'

class Fentity extends Component {
    render() {
        // const merlinLink = <Link to="/directory/wizards/merlin">Merlin</Link>
        // const morganaOther = `Trapped ${merlinLink} in a cave for eternity` 
        const match = this.props.match
        const fentitiesCategory = match.params.fentities
        const name = match.params.name
        const fentity = this.props.state[fentitiesCategory].filter(f => { 
            return f.name.toLowerCase() === name.toLowerCase() })[0]
        return (
            <div id="creature-container">
                <h1>{fentity.name}</h1>
                <img src={fentity.imgUrl} alt=""/>
                <div className="title">Power:</div>
                <div className="power text"> {fentity.power}</div>
                <div className="other text">
                    {/* {fentity.other.includes("Merlin") ? morganaOther : fentity.other} */}
                    {fentity.other}
                    </div>
            </div>
        )
    }
}

export default Fentity;
